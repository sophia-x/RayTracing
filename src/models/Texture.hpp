...
public:
	Texture(const char *file_name): __has_textures(true), __has_normals(false) {
	...
	Texture(const char *file_name, float n_radio): __has_textures(false), __has_normals(true), __n_radio(n_radio) {
		Mat image = imread(file_name, CV_LOAD_IMAGE_COLOR);
		Size s = image.size();
		__width = s.width;
		__height = s.height;
		...
		image.convertTo(__normals, CV_32FC3, 1.0 / 128);
	}

	Texture(const char *file_name, const char *normal_name, float n_radio): __has_textures(true), __has_normals(true), __n_radio(n_radio) {
		Mat image = imread(file_name, CV_LOAD_IMAGE_COLOR);
		Size s = image.size();
		__width = s.width;
		__height = s.height;
		...
		normals -= 128;
		normals.convertTo(normals_f, CV_32FC3, 1.0 / 128);
		__normals = Mat(__height, __width, CV_32FC3);
	...
	inline void getColorNormal(float u, float v, vec3 &color, vec3 &normal) const {
		...
		if (__has_normals) {
			...
			normal = normalize(vec3(tmp_n[0], tmp_n[1], tmp_n[2])) * __n_radio;
		} else {
			normal = vec3(0);
		}
	}
