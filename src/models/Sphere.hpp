...
public:
	Sphere(const vec4 &center, float radius, const vec3 &surfaceColor, float diffuse, float specular, float specular_power,
	       float reflection, float reflect_radio, float transparency, float refraction_radio):
		BasicModel(diffuse, specular, specular_power, reflection, reflect_radio, transparency, refraction_radio, model2world(center, radius),
		           false, vec3(0.0f), vec4(vec3(0), 1)), surfaceColor(surfaceColor) {}

	Sphere(const vec3 &emissionColor, const vec4 &center, float radius):
		BasicModel(0, 0, 0, 0, 0, 0, 0, model2world(center, radius), true, emissionColor, vec4(vec3(0), 1)), surfaceColor(0) {}
